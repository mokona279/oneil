...
**상태**: 완료 (P8-1·P8-2·P8-3 전부 완료) · 지시 2026-07-19 사용자
...
| P8-2 | 월 단위 시작점 전수 스윕 | 재계산(102런, 분리 프로세스) | **완료** |
...
**결과** (2026-07-19 완료, 102런 실패 0, `out/p7adv/mstart_ensemble.csv`):

gap_pp 분포 = 평균 **-20.9** · 중앙값 **-6.7** · 표준편차 58.5 ·
p5/p25/p50/p75/p95 = -183.5 / -12.4 / -6.7 / +9.8 / +29.2 ·
최선 2018-01 **+57.7** · 최악 2020-11 **-259.9**.

| 시작 연도 | 평균 gap | 최소 | 해석 |
|---|---|---|---|
| 2017 | -6.2 | -66.7 | 수렴(5·6월만 진입 타이밍 손실) |
| 2018 | **+16.7** | -1.2 | 신규 계좌가 2018 약세를 현금 통과(역방향 이득) |
| 2019 | +4.6 | -0.7 | 수렴 |
| **2020** | **-65.4** | **-259.9** | 정점 직전 진입 — 대형 음의 꼬리 |
| **2021** | **-100.3** | -211.6 | 동상 |
| 2022 | -6.7 | -7.1 | 수렴(2021 진입분과 동일 트레이드로 수렴) |
| 2023 | -4.4 | -88.2 | 대체로 수렴(6월만 이탈) |
| 2024 | -13.9 | -59.6 | 대체로 수렴 |
| 2025 | -4.0 | -9.6 | 수렴 |

**판정 — 2021-01 괴리는 "특이점"도 "상시 분포"도 아닌 레짐 조건부 꼬리다.**
gap<-50pp 17런 중 **13런이 2020-02~2021-08 연속 구간**에 집중(나머지 4런:
2017-05/06·2023-06·2024-04). 분포의 중심은 정상적 — 중앙값 -6.7pp, 절반이
±10pp 이내, IQR [-12.4, +9.8]. 즉 **평상시 시작점 운은 무시할 만하고**(헤드라인
+754.8%는 이례적 시작 운의 산물이 아님), 대형 음의 괴리는 **2020H2~2021H1
정점 직전에 현금 100%로 개시한 계좌**에만 나타난다 — 기존 경로가 이미 태운
2020 승자를 재구성할 길이 없고(P7 §8 ②) 곧바로 하락장으로 진입하기 때문.

P7 §8 5점 앙상블의 2021-01 **-227.7pp**(pre-taxfix incumbent)는 taxfix 기준에서
**-170.9pp**로 재산정되나(신규 계좌 수익 +288.5%는 양쪽 일치, incumbent만 세율
수정으로 이동), 어느 쪽이든 결론은 동일하며 **2021-01은 그 꼬리 군집의 중간값에
불과**하다(2020-11 -259.9pp가 더 나쁨). P7 §8 ②의 "성과 인용 시 2017 단일 경로
명시" 캐비앗은 유지되고, 이는 규칙 결함이 아니라 롱온리 모멘텀의 **시작 시점
시퀀스 리스크**로 문서화한다(규칙 변경 없음).

**부수 확인 — MDD는 시작점 불변**: fresh_mdd가 전 102런에서 **15.9~18.8%**로
좁게 유지(2017-22 시작 ~16%, 2023-25 시작 ~18%). -16.01% 기준 MDD가 2017 시작
운의 산물이 아님을 실증 — 개정 정책 T2 임계(-24% = -16.01%×1.5)의 근거를
독립 보강한다(시작점 운이 MDD에 더하는 폭은 최대 ~2.8pp, p95 MDD×1.5 ≈ -28%
보다 -24%가 더 보수적 → T2 현행 유지).
