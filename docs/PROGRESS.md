...
- [ ] **P2 — R3 (단계 리셋 + 4단계 감액)** ← **다음 착수**
  - 시작 컨텍스트: 계획 §4 R3(Q5 병행 확정) — (a) 4단계+ 감액 진입(목표 비중 절반) + (b) 새 사이클 리셋(N개월 무돌파 + 새 유효 베이스 깊이≥20% → 단계 1, N 스윕 축·제안 12개월). `stage_tracker` 리셋 훅 + 사이저 감액 경로 + **Q11 손절 하향 금지 클램프**(§5 확정). 비교 기준 = **v3-4**(+462.04%, ≥4× 캡처 7.08%, `out/p1/candidate_k5_alt5_b25/`). 존재 증명 표적은 계획 §1.2의 단계 영구 잠금 유형(다년 대추세에서 리셋 부재) — v3-4 전 유니버스에서 stage_ok 병목·(a)(b) 발동 빈도부터 실측. 참고: 하이닉스 2025-09-11은 v3-4에서도 수축 게이트(k=5 초과)가 차단 — R3 범위 밖. 종료 조건 동일(캡처율·가드레일·트레이드오프 표)
- [ ] **P3 — R4a (핸들 피벗)**
- [ ] **P4 — R4b (재진입) 설계 문서 → 구현**
- [ ] **P5 — 통합 워크포워드 + v4 확정**
