...
- [ ] **P3 — R4a (핸들 피벗)** ← **다음 착수**
  - 시작 컨텍스트: 계획 §4 R4a(Q10 채택 확정) — 규칙서 내부 모순 해소: 핸들 피벗 채택. 베이스 후반부(예: 마지막 1~3주)의 로컬 고점이 베이스 절대 고점보다 낮으면 그것을 피벗으로 인정 → 더 이른 진입 + 52주 밴드 간섭 완화. `base_detector` 확장(후보 구조에 핸들 고점 추적 추가, 룩어헤드 없음 유지 — 구조값 ≤d-1 확정 계약), 핸들 정의 파라미터는 config 신규 키(기본 null=현행 절대고점 피벗, 골든 불변)로. 비교 기준 = **v3-5**(+475.07%, ≥4× 캡처 7.08%, `out/p2/candidate_reset12/`). 종료 조건 동일(캡처율·가드레일·트레이드오프 표 승인) + **P2 저표본 추적 의무**: 후보·최종 실행마다 R3b 리셋 경유 진입·Q11 클램프 발동 건수·손익 기여를 분리 집계해 표에 병기
- [ ] **P4 — R4b (재진입) 설계 문서 → 구현**
- [ ] **P5 — 통합 워크포워드 + v4 확정**
