...
- [~] **P3 — R4a (핸들 피벗) — 구현·스윕 완료, 트레이드오프 표 승인 대기** (2026-07-14, 세부: `../plan/p3_handle.md`)
  ...
  - 스윕: 12조합(min_sessions {null,3,5,10}×depth {8,12,16}) 1차 소견은 **캡처 전 조합 감소** → 확장 스윕(10/15/20)에서 **min_sessions=15 캡처 무손실·20 캡처 초과(+1)** 확인(짧은 손잡이가 저품질 조기진입 남발). 후보 **min_sessions=20/12(v3-5-p3c)**: +504.13% / MDD -15.31%(=베이스) / ≥4× 캡처 7.2%(+1) / 기대값 5.04 / 311트레이드
  ...
  - **미결(사용자 승인 대상)**: 이득이 자본경합 나비효과(Q13 병목 부산물)·단일사건·s1-s2 상관 0.38이라 **채택(핸들20, 1차 지표 최선·R3b 선례)** vs **null-keep(Q13 후 재상정·R3a 선례)**가 갈리는 경계선. 저표본 추적 편입 전제 채택을 권고하되 결정은 사용자. config 기본 null 유지(승인 시 20/12·v3-6 반영)
- [ ] **P4 — R4b (재진입) 설계 문서 → 구현**
- [ ] **P5 — 통합 워크포워드 + v4 확정**
