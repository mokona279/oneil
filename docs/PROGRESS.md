...
- [ ] **U트랙 — 미국장 전이 백테스트 (2026-07-19 계획 수립)** — v4.0 무튜닝 이식으로 KR 튜닝 축들의 진짜 OOS 검증. 마스터 플랜 `us/us_plan.md` + 단계별 계획·실행 프롬프트 `us/u0~u6_*.md` (U0 결정→U1 데이터→U2 번역→U3 코드(KR 골든 비트 게이트)→U4 베이스라인→U5 강건성→U6 판정).
  - [x] **U0 — 사전 조사·결정 확정 (2026-07-20, 코드 없음)** (세부: `../plan/us/u0_decisions.md`)
    - 데이터 소스 비교표 7종(Stooq/yfinance/Tiingo/Alpha Vantage/EODHD/Norgate/Polygon) — 일봉범위·수정주가·상폐포함·일괄/증분·호출제한·비용·TOS 웹 조사(출처 병기). SEC Section 31 요율 2015~2026 시행일 계단(0.05~0.28bp = KR 거래세의 ~1/100, 25-05~26-04는 0bp) → `costs_us.yaml` 초안. FINRA TAF 주당 과금 ~0.0003bp 무시. 유니버스: NASDAQ Trader 무료 디렉터리 + 이름 접미사 파싱(보통주만)
    - **결정 Q1~Q12 + ADR 전수 승인**: Q1/Q2 **하이브리드**(Norgate 무료체험→생존편향-free 과거 base + Tiingo 증분) — 생존편향 보정 상향 채택. Q5 유동성 $7M(직접환산). Q9 커미션 0bp+슬립 5bp+Section31 계단. Q3 배당조정·Q4 지수 이중구조·Q6~Q8·Q10~Q12·ADR 포함 = 권장안대로
    - **U1 선행 검증 이월(§4.1)**: Norgate 체험판 데이터 깊이(2년 제한 우려)·라이선스 보존·조정 이음새 3건 — 체험판이 2015-10~ 전체 미확보 시 유료 1텀 or EODHD 선회
