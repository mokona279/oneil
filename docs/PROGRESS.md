...
- [ ] **P1 — R1+R2 (수축 상대화 + 템플릿 완화) + k·룩백 스윕** ← **다음 착수**
  - 시작 컨텍스트: 계획 `strategy_enhancement_plan.md` §4 R1(수축 max(피벗10%, k×ATR%))·R2a(ma200 5·20일 OR)·R2b(52주 밴드 25%) + §8 P1 종료 조건. 구현은 v3-3 config 신규 키로(기본값 = 현행과 동치, 스윕으로 켬)
  - 정렬 지표: `capture_set.csv`의 `max_multiple≥4 & turnover_ok` 부분집합을 `run_sweep(capture_symbols=...)`로 전달 → `capture_rate` 컬럼 기준. 가드레일: 삼성 트레이드 보존, MDD·기대값 트레이드오프 표
  - 비교 기준: `out/baseline_v3-3_2017_2026/` (+118.99%, 캡처율 3.2%/≥4× 3.0%). 전 유니버스 1회 243초 → 스윕 수십 조합 감당 가능, 필요 시 층화 표본
- [ ] **P2 — R3 (단계 리셋 + 4단계 감액)**
- [ ] **P3 — R4a (핸들 피벗)**
- [ ] **P4 — R4b (재진입) 설계 문서 → 구현**
- [ ] **P5 — 통합 워크포워드 + v4 확정**
